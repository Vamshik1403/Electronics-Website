...
if (contactForm) {
    const sendEmail = (e) => {
        ...
        emailjs.send('service_f4cha7k', 'template_plmv7dy', templateParams, 'zz49T4v5RaMFL8gli')
            .then(() => {
...
const sectionSlides = document.querySelectorAll('.home-slide');
let currentSlide = 0;

function showSlide(index) {
    // Remove active class from all slides
    sectionSlides.forEach(slide => slide.classList.remove('active'));
    
    // Add active class to current slide
    sectionSlides[index].classList.add('active');
}

function nextSlide() {
    currentSlide = (currentSlide + 1) % sectionSlides.length;
